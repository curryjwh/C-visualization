...
//遗传算法求解
//这里的步数相对于遗传算法而言并非移动方块来达成目标状态的步数，实际上是每一代的最优个体的变化步数，每一代结束后，若当前最优个体的曼哈顿距离小于之前的最优距离代表着算法在进化过程中找到了一个更优的状态，同时 “步数” 加 1
//populationsize表示种群大小(即每一代中个体的数量),maxgenerations表示最大迭代次数
vector<State> solveGA(const State& InitialState,int populationSize,int maxGenerations) {
    ...
    // 将初始状态作为种群的第一个个体
    population[0] = initialState;
    for (int i = 1; i < populationSize; ++i) {
        population[i] = generateRandomState();
    }

    // 用于记录算法在每一代中找到的更优状态
    vector<State> allSteps;
    allSteps.push_back(initialState);

    int bestDistance = INT_MAX;// 初始化最佳曼哈顿距离为一个极大值，用于后续比较
    State bestIndividual; // 用于存储每一代中的最佳个体
    ...
    for (int gen = 0; gen < maxGenerations; ++gen) {
        ...
        int bestIndex = 0;// 初始化最佳个体的索引为 0
        // 遍历种群，找到适应度最高的个体
        for (int i = 1; i < populationSize; i+=2) {
            if (fitnessValues[i] > fitnessValues[bestIndex]) {
                bestIndex = i; // 更新最佳个体的索引
            }
        }
        bestIndividual = population[bestIndex];
        int currentDistance = calculateManhattanDistance(bestIndividual);
        // 如果当前最佳个体的曼哈顿距离小于之前记录的最佳距离
        if (currentDistance < bestDistance) {
            bestDistance = currentDistance;
            allSteps.push_back(bestIndividual);
        }
        ...
        vector<State> newPopulation;// 用于存储下一代的种群
        // 将当前代的最佳个体直接加入到下一代种群中，进行精英保留
        ...
        for (int i = 1; i < populationSize; ++i) {
            ...
            auto children = crossover(parent1, parent2, 0.8);
            State child1 = children.first;
            State child2 = children.second;

            // 以 0.05 的概率对第子个体进行变异操作
